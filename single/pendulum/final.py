...
def init_env(initialisation = 'uniform'):
    ...
    discrete_as_size = [21] * len(as_high)
    discrete_as_win_size = (as_high\
            - as_low) / discrete_as_size
    
    print(as_high, as_low, discrete_as_size, discrete_as_win_size)
    ...
    return Q, env, os_high, os_low, as_high, as_low, discrete_os_size,\
            discrete_os_win_size, discrete_as_size, discrete_as_win_size
...
def get_continuous_action(discrete_action, discrete_as_win):
    #print(discrete_action, discrete_as_win)
    # adjust hard coded dicrete size
    continuous_action = (discrete_action - 10.5) * discrete_as_win
...
def e_greedy(Q, epsilon, dis_as, discrete_as_win, s, greedy=False):
    if greedy or np.random.rand() > epsilon:
        d_a = np.argmax(Q[s])
    else:
        d_a = np.random.randint(0, dis_as)

    a = get_continuous_action(d_a, discrete_as_win)
...
def q_lrn(alpha, gamma, epsilon, e_decay_rate, eps_start, eps_end, episodes,\
    ...
    Q, env, s_os_high, s_os_low, a_as_high, s_as_low, s_os_size, s_os_win,\
            a_as_size, a_as_win = init_env(initialisation)
    ...
    for episode in range(episodes):
        ...
        while True:
            ...
            a, d_a = e_greedy(Q, epsilon, dis_ac, a_as_win, d_s) 
    ...
    if test:
        ...
        avg_rwd, std_rwd = test_qtable(Q, env, a_as_size,\
                renderFlag, s_os_win, s_obs_low, s_as_win)
...
def sarsa(alpha, gamma, epsilon, e_decay_rate, eps_start, eps_end, episodes,\
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            s_, reward, done, info = env.step(a)
            total_reward += reward
            d_s_ = get_discrete_state(s_, env, s_obs_low, s_os_win)
...
def test_qtable(Q, env, n_tests, d_as_size, render, s_os_win, s_obs_low,\
        d_as_win, delay=0.1):
    ...
    for test in range(n_tests):
        ...
        while not done:
            ...
            a, d_a = e_greedy(Q, epsilon, d_as_size, d_as_win, d_s, greedy)
...
arr_eps = [10000, 7500, 10000, 15000]
