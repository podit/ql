...
def q_lrn(alpha, gamma, epsilon, episodes, max_steps, n_tests,\
    ...
    Q1, Q2, env , n_states, n_actions = init_env(initialisation)
    ...
    for episode in range(episodes):
        ...
        while True:
            ...
            # Generate random number to switch between Q1 and Q2
            p = np.random.random()
            
            a = e_greedy(Q1, Q2, epsilon, n_actions, s) 
            ...
            a_ = e_greedy(Q1, Q2, epsilon, n_actions, s_, greedy=True) 
            
            # If the task is completed do not use next q-value to update Q
            if done:
                # Split updates of each Q-table into 50:50 relationship 
                if p < 0.5:
                    Q1[s, a] += alpha * (reward - Q1[s, a])
                else:
                    Q2[s, a] += alpha * (reward - Q2[s, a])
            # If not done use the Bellman equation to update Q
            else:
                # Update Q based on the opposing argmax of Q
                if p < 0.5:
    ...
    if test:
        ...
        avg_rwd, avg_stp, failed = test_qtable(Q1, Q2, env, n_tests, n_actions,\
                max_steps, render)
...
def sarsa(alpha, gamma, epsilon, episodes, max_steps, n_tests,\
    ...
    for episode in range(episodes):
        ...
        while True:
            ...
            a_ = e_greedy(Q1, Q2, epsilon, n_actions, s_)
...
def test_qtable(Q1, Q2, env, n_tests, n_actions, max_steps, render, delay=0.1):
    ...
    for test in range(n_tests):
        ...
        while True:
            ...
            a = e_greedy(Q1, Q2, epsilon, n_actions, s, greedy)
